...
void run(std::string input_filename)
{
  ...
  std::shared_ptr<dyablo::AMRmesh> amr_mesh = std::make_shared<dyablo::AMRmesh>(2, 2, std::array<bool,3>({true,true,true}));
  ...
  for (int iter=0; iter<2; ++iter) {
    ...
    DataArrayBlock userdataBlock = DataArrayBlock("fake_data_block", 
                                                  bx*by, 
                                                  nbvar,
                                                  amr_mesh->getNumOctants());
