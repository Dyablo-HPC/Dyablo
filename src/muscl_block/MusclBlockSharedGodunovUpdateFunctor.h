 ...
 *
 * This functor implements the same functionality as MusclBlockGodunovUpdateFunctor,
 * but
 * - MusclBlockGodunovUpdateFunctor allocates regular Kokkos::View to store slopes
 * - MusclBlockSharedGodunovUpdateFunctor uses shared memory (aka scratch memory in kokkos,
 *   or shared memory in CUDA language) to stores slopes
 *
 * \sa MusclBlockGodunovUpdateFunctor
 */
class MusclBlockSharedGodunovUpdateFunctor {
...
public:
   ...
   *
   */
  MusclBlockSharedGodunovUpdateFunctor(std::shared_ptr<AMRmesh> pmesh,
                                       HydroParams params, 
                                       id2index_t fm,
      ...
      : pmesh(pmesh), params(params), fm(fm), blockSizes(blockSizes),
        ghostWidth(ghostWidth), nbOcts(nbOcts), nbOctsPerGroup(nbOctsPerGroup),
        iGroup(iGroup), Ugroup(Ugroup), U2(U2), Qgroup(Qgroup), dt(dt) {
  ...
  static void apply(std::shared_ptr<AMRmesh> pmesh, ConfigMap configMap,
                    ...
                    uint32_t ghostWidth, uint32_t nbOcts,
                    uint32_t nbOctsPerGroup, uint32_t iGroup,
                    DataArrayBlock Ugroup, DataArrayBlock U2,
                    DataArrayBlock Qgroup, real_t dt) {

    // instantiate functor
    MusclBlockSharedGodunovUpdateFunctor functor(
        pmesh, params, fm, blockSizes, ghostWidth, nbOcts, nbOctsPerGroup,
        iGroup, Ugroup, U2, Qgroup, dt);
    ...
    // launch computation
    Kokkos::parallel_for("dyablo::muscl::MusclBlockSharedGodunovUpdateFunctor",
                         policy, functor);
  ...
  KOKKOS_INLINE_FUNCTION HydroState
  get_cons_variables(uint32_t index, uint32_t iOct_local) const {
  ...
  template <class HydroState>
  KOKKOS_INLINE_FUNCTION HydroState
  get_prim_variables(uint32_t index, uint32_t iOct_local) const {
  ...
  // ====================================================================
  /**
   * Compute primitive variables slopes (dq) for one component from q and its
   * neighbors.
   *
  ...
  real_t slope_unsplit_scalar(real_t q, real_t qPlus, real_t qMinus) const {
   ...
   */
  KOKKOS_INLINE_FUNCTION
  real_t slope_unsplit_scalar(uint32_t ic, uint32_t ip, uint32_t im,
                              uint32_t ivar, uint32_t iOct_local) const {
    const real_t slope_type = params.settings.slope_type;
  ...
  template <class HydroState>
  KOKKOS_INLINE_FUNCTION HydroState
  slope_unsplit_hydro(const HydroState &q, const HydroState &qPlus,
   ...
   *
   * \note offsets are given in units dx/2, i.e. a vector containing only 1.0 or
   * -1.0
   *
  ...
  HydroState2d reconstruct_state_2d(const HydroState2d &q,
                                    ...
                                    const HydroState2d &dqY, offsets_t offsets,
                                    real_t dtdx, real_t dtdy) const {
    ...
    const real_t sr0 =
        (-u * drx - dux * r) * dtdx + (-v * dry - dvy * r) * dtdy;
   ...
   */
  KOKKOS_INLINE_FUNCTION
  HydroState3d reconstruct_state_3d(HydroState3d q, HydroState3d dqX,
                                    HydroState3d dqY, HydroState3d dqZ,
                                    offsets_t offsets, real_t dtdx, real_t dtdy,
                                    real_t dtdz) const {
    const double gamma = params.settings.gamma0;
    const double smallr = params.settings.smallr;
    ...
    const real_t sr0 = (-u * drx - dux * r) * dtdx +
                       ...
                       (-w * drz - dwz * r) * dtdz;
    const real_t su0 =
        (-u * dux - dpx / r) * dtdx + (-v * duy) * dtdy + (-w * duz) * dtdz;
    const real_t sv0 =
        (-u * dvx) * dtdx + (-v * dvy - dpy / r) * dtdy + (-w * dvz) * dtdz;
    const real_t sw0 =
        (-u * dwx) * dtdx + (-v * dwy) * dtdy + (-w * dwz - dpz / r) * dtdz;
    const real_t sp0 = (-u * dpx - dux * gamma * p) * dtdx +
                       (-v * dpy - dvy * gamma * p) * dtdy +
    ...
    qr[ID] =
        r + sr0 + offsets[IX] * drx + offsets[IY] * dry + offsets[IZ] * drz;
    qr[IP] =
        p + sp0 + offsets[IX] * dpx + offsets[IY] * dpy + offsets[IZ] * dpz;
    qr[IU] =
        u + su0 + offsets[IX] * dux + offsets[IY] * duy + offsets[IZ] * duz;
    qr[IV] =
        v + sv0 + offsets[IX] * dvx + offsets[IY] * dvy + offsets[IZ] * dvz;
    qr[IW] =
        w + sw0 + offsets[IX] * dwx + offsets[IY] * dwy + offsets[IZ] * dwz;
    ...
    while (iOct < iOctNextGroup and iOct < nbOcts) {
      ...
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(const int32_t index) {
            ...
            // the inner block
            if (i > 0 and i < bx1 - 1 and j > 0 and j < by1 - 1) {
              // get current location primitive variables state
              HydroState2d qprim =
                  get_prim_variables<HydroState2d>(ig, iOct_local);

              // fluxes will be accumulated in qcons
              HydroState2d qcons =
                  get_cons_variables<HydroState2d>(ig, iOct_local);
               ...
               */
              {
                ...
                HydroState2d qprim_n =
                    get_prim_variables<HydroState2d>(ig - 1, iOct_local);
               ...
               */
              {
                ...
                // get state in neighbor along X
                HydroState2d qprim_n =
                    get_prim_variables<HydroState2d>(ig - bx_g, iOct_local);
                ...
                HydroState2d qL =
                    reconstruct_state_2d(qprim_n, index - bx1, slopesX, slopesY,
                                         offsets, dtdx, dtdy);
...
}; // MusclBlockSharedGodunovUpdateFunctor
