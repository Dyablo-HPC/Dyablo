 ...
 */
PabloUniform::PabloUniform(std::string logfile, MPI_Comm comm) :
  ParaTree(logfile,comm){
  __reset();
}
#else
PabloUniform::PabloUniform(std::string logfile) :
  ParaTree(logfile) {
  __reset();
}
#endif
 ...
 * \param[in] comm The MPI communicator used by the parallel octree. MPI_COMM_WORLD is the default value.
 */
PabloUniform::PabloUniform(uint8_t dim, std::string logfile, MPI_Comm comm) :
  ParaTree(dim,logfile,comm){
  __reset();
};
#else
PabloUniform::PabloUniform(uint8_t dim, std::string logfile) :
  ParaTree(dim,logfile){
  __reset();
};
#endif
 ...
 * \param[in] comm The MPI communicator used by the parallel octree. MPI_COMM_WORLD is the default value.
 */
PabloUniform::PabloUniform(double X, double Y, double Z, double L,
			   uint8_t dim, std::string logfile,
			   MPI_Comm comm) :
  ParaTree(dim,logfile,comm) {
  __reset();
