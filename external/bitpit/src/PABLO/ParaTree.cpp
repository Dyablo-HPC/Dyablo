 ...
 * \param[in] logfile The file name for the log of this
 * object. PABLO.log is the default value.
 */
#if BITPIT_ENABLE_MPI==1
/*!
 * \param[in] comm The MPI communicator used by the parallel
 * octree. MPI_COMM_WORLD is the default value.
 */
...
ParaTree::initialize(const std::string &logfile, MPI_Comm comm) {
  ...
  void
    ...
    /*!Get the last operation perforfmed by the octree
     * (initialization, adapt (mapped or unmapped), loadbalance (first
     * or not). 
	...
	*previousCommunicator = m_comm;
    ...
    /*! Get the edge-face connectivity for 12 edge (in 2D case not to
     * be considered at all). 
     * \param[out] edgeface Connectivity edge-face. edgeface[i][0:1] =
     * local indices of faces sharing the i-th edge of an octant.
     */
    void
    ...
    /*! Get the components (in logical domain) of the 6 normals to the
     * faces of an octant (for the 2D case consider only the first 4) 
    ...
    /*! Get the indices of the faces of virtual octants opposed to the
     * 6 faces of an octant (for the 2D case consider only the first 4 terms).
     * \return Pointer to opposed faces array[6] to the faces of an
     * octant (oppface[i] = Index of the face of an octant neighbour
    ...
    /*! Get the face-node connectivity for 6 faces (in 2D case
     * consider only the first 4 terms). 
     * \return Pointer to connectivity face-node. facenode[i][0:1] =
